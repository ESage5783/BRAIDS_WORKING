...
Accelpath <- paste(datpath, 'Accelerometry', sep = '')
Presspath <- paste(datpath, 'Pressure', sep = '')
```
...
MetaData <- subset(MetaData, Colony == 'DUN')
...
```{R write to csv}
write.csv(GPS_out, file = paste(savepath, 'DUN_2025_KITTI_Tracks.csv', sep = ''), row.names=FALSE)
