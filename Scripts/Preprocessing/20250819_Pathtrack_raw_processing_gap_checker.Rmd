...
datpath <- "C:/Users/ElspethSage/OneDrive - THE ROYAL SOCIETY FOR THE PROTECTION OF BIRDS/Documents/BRAIDS/WORKING/Data/Tracks_processed/KITTI/"
...
p <- plot_grid(p1, p2, labels = c('A', 'B'), label_size = 12)
    ...
    title <- paste(savepath,'/Figures/battery_timegaps_2025_3/SAH_', date(temp$date_time[1]),'_', date(temp$date_time[nrow(temp)]),'_logger_', unique(GPS$Tag_ID)[i], '.png', sep = '') 
...
```{R battery level per plot}
p <- ggplot()+
  geom_line(data = GPS, aes(x = date_time, y = Battery, group = Tag_ID, colour = as.factor(Tag_ID)), alpha = 0.5)+
  facet_wrap(~Plot)

title <- paste(savepath,'/Figures/2025_SAH_plot_battery.png', sep = '')
ggsave(title,plot = p,width = 15, height = 6, dpi = 150, units = "in", device='png')
