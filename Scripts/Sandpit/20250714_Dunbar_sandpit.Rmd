...
```{R read in GPS accel pressure, warning = FALSE, message = FALSE}
...
Accel <- read.csv(paste(datpath, '/Accelerometry_processed/KITTI/DUN_KITTI_2025_Accel.csv', sep = ''), header = TRUE, sep = ",", stringsAsFactors=FALSE, row.names = NULL)
Press <- read.csv(paste(datpath, '/Pressure_processed/KITTI/DUN_KITTI_2025_Pressure.csv', sep = ''), header = TRUE, sep = ",", stringsAsFactors=FALSE, row.names = NULL)
```

```{R compare two DUN datasets, warning = FALSE, message = FALSE}
...
```{R further basic filtering, warning = FALSE, message = FALSE}

# MoverakeR names
GPS$TagID <- GPS$Tag_ID
GPS$DateTime <- GPS$date_time
...
```{R moverakeR play, warning = FALSE, message = FALSE}
indata_ctmm <- ctmm::as.telemetry(GPS)
OUT <- ctmm::outlie(indata_ctmm)
plot(OUT,units=FALSE)

GPS$satellites_used <- GPS$Satellites
GPS<- clean_GPS(GPS)
...
lef
saveWidget(lef, file=paste(savepath, '/Leaflet_maps/DUN_ALL_MoveRakeRFilt',".html", sep = ''))
...
count <- 1
for(i in unique(GPS$TagID)){
  lefplot <- subset(GPS, TagID == i)

  pal <-  c("green", "darkgreen","blue","navy","red","deeppink4","purple","orange","magenta","violet")
...
lef <- leaflet() %>%
  ...
  lef
  saveWidget(lef, file=paste(savepath, '/Leaflet_maps/TimeSlider_lefmap_',lefplot$Tag_ID[1],".html", sep = ''))

  count <- count +1
}
```

```{R Accel & press time gaps, warning = FALSE, message = FALSE}

Accel$date_time <- as_datetime(paste(Accel$Year, '/' ,Accel$Month, '/', Accel$Day, '/', Accel$Hour, ':', Accel$Minute, ':', Accel$Second))

Accel_out <- c()
for(i in 1:length(unique(Accel$TagID))){
  Accel_i <- subset(Accel, TagID %in% unique(Accel$TagID)[i])
  Accel_i <- Accel_i[order(Accel_i$date_time),]
  Accel_i$timeinterval_f <- c(NA,difftime(Accel_i$date_time[-1], Accel_i$date_time[-length(Accel_i$date_time)], units = "secs"))

  Accel_out <- rbind(Accel_out, Accel_i)
}


Accelsumm <- Accel_out %>% dplyr::group_by(TagID) %>% dplyr::summarise(nfixes = length(date_time),
                                                  start_time = min(date_time),
                                                  end_time = max(date_time),
...
Press_out <- c()
for(i in 1:length(unique(Press$TagID))){
  Press_i <- subset(Press, TagID %in% unique(Press$TagID)[i])
  Press_i <- Press_i[order(Press_i$date_time),]
  Press_i$timeinterval_f <- c(NA,difftime(Press_i$date_time[-1], Press_i$date_time[-length(Press_i$date_time)], units = "secs"))

  Press_out <- rbind(Press_out, Press_i)
}


Pressumm <- Press_out %>% dplyr::group_by(TagID) %>% dplyr::summarise(nfixes = length(date_time),
                                                  start_time = min(date_time),
                                                  end_time = max(date_time),
                                                   minInterval = min(timeinterval_f, na.rm = TRUE), maxInterval = max(timeinterval_f, na.rm = TRUE), medianInterval = median(timeinterval_f, na.rm = TRUE))
...
```{R play with one individual, warning = FALSE, message = FALSE}
IndBack <- subset(Accel, TagID == 35844)

temp <- subset(Accel, TagID == 35844) %>% dplyr::select(Zaccel, Xaccel, Yaccel, date_time)
xtsdatAcc <- xts(temp, order.by = temp$date_time, tz="UTC")

dyAcc <-dygraph(xtsdatAcc, main = "Acceleration",group = "tagID 35854") %>%
...
IndTail <- subset(Accel, TagID == 35903)

temp <- subset(Accel, TagID == 35903) %>% dplyr::select(Zaccel, Xaccel, Yaccel, date_time)
xtsdatAcc <- xts(temp, order.by = temp$date_time, tz="UTC")
...
ggplot(data = IndBack)+
  geom_path(aes(x = date_time, y = X3Daccel), alpha = 0.2)+
  geom_point(aes(x = date_time, y = X3Daccel), alpha = 0.2)+
  scale_x_datetime(breaks = "hours")
...
Press$day <- day(Press$date_time)

Ns <- unique(Press$TagID)

for(i in Ns){
  dataN <- subset(Press, TagID == i)
    ...
    title <- paste(savepath,'/Figures/Pressurefigures/2Dunbar_', date(dataN$date_time[1]),'_', date(dataN$date_time[nrow(dataN)]),'_logger_', i, '.png', sep = '')
    ggsave(title,plot = p,width = 15, height = 6, dpi = 150, units = "in", device='png')
  }

# plot strange tag day bday
press35853_1 <- filter(Press, tagID == 35853 & pressure < 3000)
press35853_2 <- filter(Press, tagID == 35853 & pressure >= 3000)
...
for(i in Ns){
  ...
  p1 <- ggplot(dataN)+
    geom_point(aes(x = date_time, y = Zaccel, color = "z accel"), alpha = 0.1, stroke = NA, fill = 'black')+
    geom_point(aes(x = date_time, y = Yaccel, color = "y accel"), alpha = 0.1, stroke = NA, fill = 'red')+
    geom_point(aes(x = date_time, y = Xaccel, color = "x accel"), alpha = 0.1, stroke = NA, fill = 'blue')+
    theme_bw(base_size = 10)+
    labs(title = (paste('Dunbar, ', date(dataN$date_time[1]), ':', date(dataN$date_time[nrow(dataN)]), ' TagID: ', i, sep = '')))+
    # scale_x_datetime(date_minor_breaks = "1 hour", date_breaks = "1 hour",date_labels = "%m/%d %H:%M") +
    # theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.5))+
      theme(text = element_text(size = 19), axis.title.x=element_blank(),
    ...
    p2 <- ggplot(dataN)+
    geom_point(aes(x = date_time, y = X3Daccel), alpha = 0.1, stroke = NA, color = "black", fill = 'black')+
    theme_bw(base_size = 10)+
    # labs(title = (paste('Flight trial, ', date(dataN$date_time[1]), ' TagID: ', i, sep = '')))+
    scale_x_datetime(breaks = "hours")+
    scale_x_datetime(date_minor_breaks = "1 hour", date_breaks = "1 hour",date_labels = "%m/%d %H:%M") +
    theme(text = element_text(size = 19), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.5))+
    xlab('')+
    ylab('X3Daccel [m/s]')
  
    p <- plot_grid(p1,p2, ncol = 1, align = "v")
    
  title <- paste(savepath,'/Figures/Accelerometerfigures/Dunbar_2025_', i, '.png', sep = '')
  ggsave(title,plot = p,width = 20, height = 15, dpi = 150, units = "in", device='png')
}
...
for(i in Ns){
  ...
  title <- paste(savepath,'/Figures/Accelerometerfigures/Dunbar_2025_X3Daccel_', i, '.png', sep = '')
  ggsave(title,plot = p,width = 15, height = 6, dpi = 150, units = "in", device='png')
}
```
...
tempp <- subset(Press, tagID == 35903) %>% dplyr::select(pressure, date_time)
temp <- subset(Accel, TagID == 35844) %>% dplyr::select(Zaccel, Xaccel, Yaccel, date_time)
xtsdatAcc <- xts(temp, order.by = temp$date_time, tz="UTC")
